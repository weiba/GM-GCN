...
def load_data(path):
    # load network
    adj = sp.load_npz(path + "PR.adj.npz")      # gene-miRNA network
    network = adj.tocsc()

    # load node features
    feat1 = pd.read_csv(path + "P.feat-pre.csv", sep=",").values[:, 1:]     # gene
    l_feature = torch.Tensor(feat1).to(device)

    feat2 = pd.read_csv(path + "R.feat-pre.csv", sep=",").values[:, 1:]         # miRNA
    r_feature = torch.Tensor(feat2).to(device)

    # load original features
    feat3 = pd.read_csv(path + "P.feat-final.csv", sep=",").values[:, 1:]  # miRNA
    ori_feature = torch.Tensor(feat3).to(device)
    ...
    return network, l_feature, r_feature, ori_feature, k_sets, Y
...
if __name__ == '__main__':
    ...
    # path = "./data/cancer name/"     # specific cancer,e.g.brca
    network, l_feature, r_feature, ori_feature, k_sets, Y = load_data(path)
    ...
    for i in range(10):
        ...
        for cv_run in range(5):
            ...
            # load model
            model = Net(l_feature, r_feature, ori_feature, network, 1, 64, 256, 128).to(device)    # hop=1
    ...
    #         model = Net(l_feature, r_feature, ori_feature, network, 1, 19, 256, 128).to(device)     # hop=1
